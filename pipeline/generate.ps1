...
$inputPath = Join-Path $root $config.input; $staging = Join-Path $root '.local\output'; $final = Join-Path $root $config.output
New-Item -ItemType Directory -Force -Path $staging,(Split-Path -Parent $final) | Out-Null
function Run-SF3D {
  $repo = Join-Path $root '.local\stable-fast-3d'; $python = Join-Path $repo '.venv-runtime\Scripts\python.exe'; if (!(Test-Path $python)) { throw 'SF3D environment missing. Run npm run model:install.' }
...
function Run-TripoSR {
  $repo = Join-Path $root '.local\TripoSR'; $python = Join-Path $repo '.venv-runtime\Scripts\python.exe'; if (!(Test-Path $python)) { throw 'TripoSR environment missing. Run npm run model:install.' }
