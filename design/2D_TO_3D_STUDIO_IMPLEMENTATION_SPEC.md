...
在用户上传正面、侧面、背面等原始素材后，系统可以增加一个可选的“AI 细节补充”阶段。该阶段对人物进行语义区域分析，为证据不足或分辨率不足的区域生成受约束的多视角细节候选，并在人工批准后形成新的不可变 Reference Set。

该能力的目标是增加可用于几何重建、局部雕刻、法线烘焙和材质生成的视觉证据，而不是通过放大图片或增加网格面数虚构精度。AI 生成内容必须标记为推断证据，不能与用户上传的原始证据混淆。
...
原始三视图入库
→ 跨视图一致性与覆盖度分析
→ 面部/头发/衣领/躯干/肩袖/手部/背部/配饰区域划分
→ 为每个区域输出补图计划、目标视角、用途和风险
→ 用户选择区域及保守/平衡/创作模式
→ AI 联合生成 2–4 个多视角候选组
→ 用户逐组批准、拒绝或重试
→ 批准结果写入新的不可变 Reference Set
→ 生成粗模并执行固定视角重投影比较
→ 将差异区域路由到几何、法线/置换或纹理流程
→ Blender 局部精修与最终验收
```

核心规则：

- 原始资产永不覆盖；每次批准创建新的 Reference Set 版本。
- 区域候选必须作为一组审批，不允许从互相矛盾的候选中随意拼接视角。
- 每张资产标记 `observed`、`constrained`、`inferred` 或 `designed` 证据等级。
- 每项细节标记目标用途：`geometry`、`normal_displacement` 或 `material`。
- 未经批准的候选不能进入 Hunyuan、Blender 或纹理任务。
- 当前多视图后端不支持的额外视图转入局部精修，不得声称已作为几何模型条件输入。
- AI 不确定性高且影响身份、轮廓或结构时，优先请求用户补充真实素材。

建议的首版语义区域为：头部整体、面部、头发/发髻、颈部与衣领、躯干服装、左右肩袖、手臂和手、下半身、背部结构、独立配饰。第一版不进行无约束的几十块自动碎片化切分。

三种生成模式：

| 模式 | 行为 | 使用场景 |
|---|---|---|
| 保守 | 裁切、放大、降噪和低强度局部修复 | 身份保持和严格还原 |
| 平衡 | 允许补充 45° 视角和少量遮挡区域 | 默认生产模式 |
| 创作 | 允许设计不可见结构并显著标注为推断 | 概念设计或用户明确授权 |

预期影响：面部、衣领、袖口、发型和背面结构的可恢复程度提高；纹理和材质完整度明显提高；拓扑质量不会因补图自动提高，仍需几何清理、局部重建或重拓扑。逐张独立生成、过高重绘强度或未经审核的隐藏面推断可能造成身份漂移、左右不一致、结构幻觉和错误几何。
